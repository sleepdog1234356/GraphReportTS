...
`no_hankel_graph` reads the sequence cache under `BATTERY_SEQUENCE_CACHE_DIR`; graph variants keep using `BATTERY_GRAPH_CACHE_DIR`. Before training, the runner checks prompt/target identity across the representation-specific caches. The launcher defaults `FULL_REFERENCE_COMMIT` to `1d6a8f975fd3225cc087af90f03a00414ce84591`, the deployed full v3 source snapshot; this identity is not the active asset-tree HEAD, which may include unrelated commits or working-tree changes. An explicit `FULL_REFERENCE_COMMIT` must be a non-empty 40-hex commit. A different valid commit additionally requires `ALLOW_FULL_REFERENCE_COMMIT_OVERRIDE=1` so an operator cannot silently mislabel reused results. Set `ABLATION_FORCE_RETRAIN=1` only to discard and retrain the four core variants; the `full` row is still reused. `ABLATION_CODE_ROOT` may explicitly select the implementation worktree when needed.

Dry-run does not require full artifacts: `bstalignment.run_core_ablation_suite --dry_run` validates the formal CLI and prints exactly 12 training commands before artifact/cache validation, so it is safe while the full stage is unfinished. By contrast, non-dry execution still requires completed full artifacts for MIT, CALCE, and XJTU and validates them before any reuse.
