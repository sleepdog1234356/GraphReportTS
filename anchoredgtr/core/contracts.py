...
class GTRConfig:
    ...
    def __post_init__(self) -> None:
        ...
        if self.text_backend == "distilbert" and not self.freeze_text:
            raise ValueError("the approved GTR DistilBERT backbone must remain frozen")
...
class ForecastBatch:
    ...
    def to(self, device: torch.device | str, non_blocking: bool = False) -> "ForecastBatch":
        return _move_dataclass(self, device, non_blocking)

    def pin_memory(self) -> "ForecastBatch":
        return _pin_dataclass(self)


@dataclass
class BatteryRawBatch:
    ...
    def to(self, device: torch.device | str, non_blocking: bool = False) -> "BatteryRawBatch":
        return _move_dataclass(self, device, non_blocking)

    def pin_memory(self) -> "BatteryRawBatch":
        return _pin_dataclass(self)
...
def validate_battery_raw_batch(batch: BatteryRawBatch) -> None:
