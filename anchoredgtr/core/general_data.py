...
from .contracts import ForecastBatch
from .prompts import (
...
GENERAL_PREPROCESSING_SCHEMA = "anchored-gtr-general-preprocessing-v1"
GENERAL_PROMPT_SCHEMA = "anchored-gtr-general-prompt-v1"
...
class GeneralForecastDataset(Dataset):
    ...
    def __init__(
        ...
        if self.scaler.mean is None or self.scaler.std is None:
            raise ValueError("AnchoredGTR requires a train-fitted scaler")
        ...
        if prompt_thresholds is None:
            raise ValueError("AnchoredGTR requires train-fitted prompt_thresholds")
        ...
        self._prompt_cache: dict[int, PromptResult] = {}

    def __len__(self) -> int:
        return len(self.samples)

    def _prompt_result_at(self, index: int) -> PromptResult:
...
class SyntheticGeneralDataset(Dataset):
    def __init__(
        self,
        size: int = 32,
        variables: int = 7,
...
def collate_general(items: Sequence[dict[str, object]]) -> ForecastBatch:
    if not items:
