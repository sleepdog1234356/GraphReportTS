...
from .battery_data import BatteryForecastDataset, collate_battery
from .contracts import GTRConfig, require_local_text_model
from .losses import battery_gtr_loss
from .model import BatteryGTRCore
...
BATTERY_GTR_TRAINING_PROTOCOL = "battery-gtr-plateau-guarded-v1"
BATTERY_GTR_CORRECTED_MODEL_NAME = "BatteryGTR"
...
def _checkpoint_training_state(
    *,
    ...
    stale: int,
    lr_reductions: int,
    training_protocol: str = BATTERY_GTR_TRAINING_PROTOCOL,
) -> dict[str, Any]:
    return {
...
def run_epoch(
    ...
    *,
    ...
    with torch.set_grad_enabled(training):
        for raw_batch in loader:
            ...
            with torch.autocast(device_type=device.type, dtype=amp_dtype, enabled=amp_enabled):
                ...
                losses = battery_gtr_loss(output, raw_batch.target, raw_batch.target_mask, align_weight_value)
            if training:
    ...
    if count == 0:
        raise RuntimeError("BatteryGTR data loader produced no batches")
...
def build_datasets(args: argparse.Namespace) -> tuple[
    ...
    validation = BatteryForecastDataset(
        cache,
        split="val",
        seed=args.seed,
        prompt_mode=args.prompt_mode,
    ...
    if not train.split_definition == validation.split_definition == test.split_definition:
        raise RuntimeError("BatteryGTR train, validation, and test datasets disagree on cell split")
    if min(len(train), len(validation), len(test)) == 0:
        raise RuntimeError("BatteryGTR requires non-empty train, validation, and test splits")
    return cache, train, validation, test


def _loader(dataset: BatteryForecastDataset, args: argparse.Namespace, shuffle: bool) -> DataLoader:
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Train BatteryGTR on cached battery sensor features")
    parser.add_argument("--dataset", choices=("mit", "xjtu", "synthetic"), default="mit")
    parser.add_argument("--cache_dir", required=True)
    parser.add_argument("--output", default="artifacts/battery/battery_gtr/runs")
...
def main(
    ...
    *,
    model_factory: Callable[[GTRConfig], BatteryGTRCore] = BatteryGTRCore,
    model_config_overrides: Mapping[str, Any] | None = None,
    model_name: str = BATTERY_GTR_CORRECTED_MODEL_NAME,
    training_protocol: str = BATTERY_GTR_TRAINING_PROTOCOL,
    model_prepare_hook: Callable[..., dict[str, Any]] | None = None,
) -> None:
    args = parse_args(argv)
    if args.dataset == "xjtu" and not Path(args.cache_dir).exists():
        raise ValueError("XJTU is deferred; only an existing compatible GTR cache may be used")
    ...
    model_config = GTRConfig(**model_config_values)
    ...
    training_config = GTRTrainingConfig(
    ...
    if args.resume and last_path.exists() and not args.smoke:
        ...
        if stable_digest(checkpoint.get("run_identity", {})) != stable_digest(run_identity):
            raise RuntimeError("battery-gtr resume checkpoint identity differs from the requested run")
        if checkpoint.get("scheduler") is None:
            raise RuntimeError("battery-gtr resume checkpoint is missing scheduler state")
    ...
    for epoch in range(start_epoch, training_config.epochs + 1):
        ...
        if should_stop_gtr(stale, lr_reductions, training_config):
            print(
