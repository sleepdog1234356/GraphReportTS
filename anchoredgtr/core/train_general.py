...
from .contracts import GTRConfig
from .general_data import GeneralForecastDataset, SyntheticGeneralDataset, collate_general
from .losses import general_gtr_loss
from .model import GraphTextResidualCore
...
GENERAL_TRAINING_STRATEGY_VERSION = "anchored-gtr-adaptive-sparse-graph-compact-prompt-v3"


def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Train AnchoredGTR on general multivariate datasets")
    ...
    parser.add_argument("--model_name", default="AnchoredGTR")
...
LINEAR_ANCHOR_CONFIG_SCHEMA = "gtr-linear-anchor-v1"
...
def _sample_branch_gradient_norms(model: GraphTextResidualCore) -> dict[str, float]:
    ...
    for batch_index, batch in enumerate(loader, start=1):
        ...
        with torch.set_grad_enabled(training):
            with torch.autocast(device_type=device.type, dtype=amp_dtype, enabled=amp_dtype is not None):
                output = model(batch)
                losses = general_gtr_loss(output, batch.target, batch.target_mask, align)
            if training:
...
def main(
    ...
    *,
    ...
    if not isinstance(preprocessing, dict) or not isinstance(preprocessing.get("dataset_identity"), dict):
        raise RuntimeError("AnchoredGTR training dataset did not provide dataset identity")
    ...
    factory = GraphTextResidualCore if model_factory is None else model_factory
    ...
    if args.init_model_checkpoint:
        ...
        initialization_audit = {
            "schema": "gtr-model-only-initialization-v1",
    ...
    run_config: dict[str, Any] = {
        "schema": "gtr-general-run-config-v1",
        "mode": args.mode,
        "arguments": vars(args),
    ...
    for epoch in range(start_epoch, training.epochs + 1):
        ...
        if epoch >= training.warmup_epochs:
            ...
            if lr_reduced:
                lr_reductions += 1
        early_stop_eligible = should_stop_gtr(stale, lr_reductions, training)
        record = {
            "epoch": epoch,
            "align_weight": align,
            "train": train_metrics,
    ...
    if args.skip_final_test:
        pilot_summary = {
            "schema": "gtr-validation-only-pilot-v1",
            "best_val_mse": best_mse,
            "best_epoch": best_epoch,
            "epochs": len(history),
