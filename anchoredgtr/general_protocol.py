...
class GeneralWindowSpec:
    input_len: int
    prompt_len: int
    horizons: tuple[int, ...]


_GENERAL_WINDOW_SPECS = {
    (36, 36): GeneralWindowSpec(36, 36, FORMAL_HORIZONS),
    (96, 96): GeneralWindowSpec(96, 96, (96, 192, 336, 720)),
}


def window_spec(input_len: int, prompt_len: int) -> GeneralWindowSpec:
    key = (int(input_len), int(prompt_len))
    try:
        return _GENERAL_WINDOW_SPECS[key]
    except KeyError as exc:
        raise ValueError(
            f"unsupported general window protocol: input={key[0]} prompt={key[1]}"
        ) from exc
...
def split_bounds(dataset: str, n_rows: int, input_len: int) -> dict[str, tuple[int, int]]:
    ...
    if int(input_len) not in {spec.input_len for spec in _GENERAL_WINDOW_SPECS.values()}:
        raise ValueError("formal general forecasting input_len must be 36 or 96")
...
class GeneralForecastProtocol:
    ...
    def __init__(
        ...
        self.spec = window_spec(input_len, prompt_len)
        self.input_len = self.spec.input_len
        self.prompt_len = self.spec.prompt_len
    ...
    def window_index(self, split: str, pred_len: int) -> np.ndarray:
        ...
        if pred_len not in self.spec.horizons:
            raise ValueError(
                f"unsupported formal prediction length: {pred_len}; expected one of {self.spec.horizons}"
            )
