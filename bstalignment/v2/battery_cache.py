"""Versioned on-disk cache for deterministic GraphReportTS-v2 battery inputs."""
...
CACHE_SCHEMA = "battery_features_v2"
...
class BatteryFeatureCache:
    """Lazy per-cell reader/writer for the deterministic battery-v2 cache."""
    ...
    def open(
        ...
        if not manifest_path.exists():
            raise FileNotFoundError(f"battery v2 cache manifest not found: {manifest_path}")
        ...
        if stored_hash != actual_hash:
            raise ValueError("battery v2 cache manifest checksum mismatch")
        ...
        for key, expected_value in expected.items():
            if manifest.get(key) != expected_value:
                raise ValueError(
                    f"battery v2 feature schema mismatch: {key} expected {expected_value!r}, "
                    f"got {manifest.get(key)!r}"
                )
        cells = manifest.get("cells")
        if not isinstance(cells, dict) or not cells:
            raise ValueError("battery v2 cache has no cells")
        for cell_id, info in cells.items():
            if not isinstance(info, dict) or "file" not in info:
                raise ValueError(f"battery v2 cache cell entry is invalid: {cell_id}")
            path = (root / info["file"]).resolve()
            if path.parent != root or not path.exists():
                raise ValueError(f"battery v2 cache cell file is missing or unsafe: {cell_id}")
    ...
    def create(
        ...
        *,
        ...
        root = Path(root).expanduser().resolve()
        if not cells:
            raise ValueError("cannot create an empty battery v2 cache")
