...
from .contracts import BatteryRawBatchV2, validate_battery_raw_batch
...
class BatteryFeatureScalerV2:
    ...
    @classmethod
    def fit(cls, cells: Iterable[Any]) -> "BatteryFeatureScalerV2":
    ...
    @classmethod
    def from_state_dict(cls, state: dict[str, Any]) -> "BatteryFeatureScalerV2":
        return cls(
...
@dataclass(frozen=True)
class BatterySplitV2:
    train: tuple[str, ...]
    val: tuple[str, ...]
    test: tuple[str, ...]

    @classmethod
    def from_cell_ids(cls, cell_ids: Sequence[str], seed: int) -> "BatterySplitV2":
    ...
    @classmethod
    def from_cache(cls, cache: BatteryFeatureCache, seed: int) -> "BatterySplitV2":
...
class BatteryForecastV2Dataset(Dataset):
    ...
    def __init__(
        ...
        *,
        ...
        scaler: BatteryFeatureScalerV2 | None = None,
        ...
        self.epoch = 0
        self.split_definition = BatterySplitV2.from_cache(cache, seed=self.seed)
        train_cells = [cache.load_cell(cell_id) for cell_id in self.split_definition.train]
        self.scaler = scaler or BatteryFeatureScalerV2.fit(train_cells)
        ...
        if missing_thresholds:
            raise ValueError(f"battery v2 prompt thresholds are missing: {missing_thresholds}")
        ...
        if any(len(values) != 2 for values in self.prompt_thresholds.values()):
            raise ValueError("each battery v2 prompt threshold requires two train-fitted boundaries")
        self.cells = {cell_id: cache.load_cell(cell_id) for cell_id in self.split_definition.ids(split)}
        self.samples: list[BatteryWindowV2] = []
        minimum = BATTERY_PROMPT_CYCLES + BATTERY_NUMERIC_CYCLES + BATTERY_PREDICTION_CYCLES
            ...
            for segment_index, (segment_start, segment_end) in enumerate(_consecutive_segments(observations)):
                ...
                for origin in range(first_origin, final_origin + 1):
                    ...
                    if int(valid_v_i.sum()) < BATTERY_MIN_VALID_NUMERIC_CYCLES or not np.isfinite(future).all():
                        continue
                    self.samples.append(
                        BatteryWindowV2(cell_id, origin, segment_index, segment_start, segment_end)
                    )
        ...
        if len(self.sample_keys) != len(set(self.sample_keys)):
            raise RuntimeError("battery v2 forecast windows produced duplicate stable keys")
...
def collate_battery_v2(items: Sequence[dict[str, Any]]) -> BatteryRawBatchV2:
    if not items:
        raise ValueError("cannot collate an empty battery v2 batch")
    batch = BatteryRawBatchV2(
