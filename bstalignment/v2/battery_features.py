"""Sensor-only, physics-preserving per-cycle battery features for GraphReportTS-v2.
...
FEATURE_SCHEMA_VERSION = "battery_sensor_features_v2.1.0"
CURVE_POINTS = 128
CURVE_AXIS_SCHEMA_VERSION = "battery_curve_axes_v2.1.0"
...
def _empty_curve(kind: str, points: int) -> DerivativeCurve:
    if points != CURVE_POINTS:
        raise ValueError(f"battery v2 derivative curves require exactly {CURVE_POINTS} points")
    if kind == "ic":
...
def compute_ic_curve(signals: CycleSignals, points: int = CURVE_POINTS) -> DerivativeCurve:
    ...
    if points != CURVE_POINTS:
        raise ValueError(f"battery v2 IC curves require exactly {CURVE_POINTS} points")
...
def compute_dv_curve(signals: CycleSignals, points: int = CURVE_POINTS) -> DerivativeCurve:
    ...
    if points != CURVE_POINTS:
        raise ValueError(f"battery v2 DV curves require exactly {CURVE_POINTS} points")
    source = _charge_capacity(signals)
    if source is None:
