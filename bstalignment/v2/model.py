...
from .semantic import FrozenDistilBERTEncoder, GatedSemanticFusionV2


class GraphReportTSv2(nn.Module):
    ...
    def __init__(self, config: GraphReportTSv2Config) -> None:
        ...
        if config.use_text:
            ...
            self.semantic_fusion = GatedSemanticFusionV2(config.d_model, config.dropout, initial_gate=0.4)
        else:
    ...
    def forward(self, batch: ForecastBatchV2) -> dict[str, torch.Tensor | object]:
        validate_batch(
            batch,
            self.config.domain,
...
class BatteryGraphReportTSv2(nn.Module):
    """Defined here as an integration boundary; the adapter is imported lazily."""

    def __init__(self, config: GraphReportTSv2Config) -> None:
        super().__init__()
        if config.domain != "battery":
            raise ValueError("BatteryGraphReportTSv2 requires domain=battery")
        from .battery_adapter import BatteryFeatureAdapterV2

        self.feature_adapter = BatteryFeatureAdapterV2()
        self.shared = GraphReportTSv2(config)

    def forward(self, raw_batch: BatteryRawBatchV2):
        ...
        forecast_batch = ForecastBatchV2(
