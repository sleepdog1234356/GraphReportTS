...
RESULT_SCHEMA_VERSION = "graph-report-ts-v2-results-v1"
...
class V2ResultRecord:
    ...
    def validate(self) -> None:
        ...
        if self.domain == "battery" and self.task != "battery_M_to_1":
            raise ValueError("battery v2 results must use task=battery_M_to_1")
        if self.horizon <= 0:
...
def write_result(record: V2ResultRecord, run_dir: str | Path) -> Path:
...
def load_result(path: str | Path) -> dict[str, Any]:
    ...
    if payload.get("schema_version") != RESULT_SCHEMA_VERSION:
        raise ValueError(f"unsupported v2 result schema in {path}")
    return payload
...
def aggregate_results(result_paths: Iterable[str | Path], output_csv: str | Path) -> Path:
    ...
    if not rows:
        raise ValueError("at least one v2 result is required for aggregation")
    scalar_fields = (
        "schema_version",
        "model",
        "domain",
