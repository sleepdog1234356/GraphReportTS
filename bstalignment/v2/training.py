...
def alignment_weight(epoch: int, config: V2TrainingConfig) -> float:
...
def build_optimizer(model: torch.nn.Module, config: V2TrainingConfig) -> torch.optim.Optimizer:
    semantic, core = [], []
...
def apply_warmup(optimizer: torch.optim.Optimizer, epoch: int, config: V2TrainingConfig) -> None:
...
def should_stop_v2(stale: int, lr_reductions: int, config: V2TrainingConfig) -> bool:
    return (
