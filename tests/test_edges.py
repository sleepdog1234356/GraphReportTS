...
from anchoredgtr.core.edges import (
    RELATIONS,
    SparseEdgeBuilder,
    _difference_correlation,
)
from anchoredgtr.core.contracts import GTRConfig
from anchoredgtr.core.model import BatteryGTRCore, GraphTextResidualCore
from anchoredgtr.core.patching import AdaptivePatchifier
...
class AdaptiveVariableNeighborTests(unittest.TestCase):
    ...
    def test_general_and_battery_models_share_the_same_edge_builder(self):
        general = GraphTextResidualCore(
            GTRConfig(domain="general", input_len=36, pred_len=24, use_text=False)
        )
        battery = BatteryGTRCore(
            GTRConfig(domain="battery", input_len=32, pred_len=20, use_text=False)
        )
