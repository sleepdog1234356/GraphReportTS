...
from anchoredgtr.core.train_general import (
...
from anchoredgtr.core.training import (
    GTRTrainingConfig,
    apply_warmup,
    build_optimizer,
...
def _role_optimizer(config: GTRTrainingConfig) -> torch.optim.Optimizer:
...
class LinearAnchorInitializationTests(unittest.TestCase):
    ...
    def test_frozen_anchor_parameters_are_excluded_from_optimizer(self):
        ...
        optimizer = build_optimizer(model, GTRTrainingConfig())
...
class GTRWarmupAndPlateauTests(unittest.TestCase):
    ...
    def test_plateau_reduces_both_roles_and_reports_actual_reductions(self):
        config = replace(GTRTrainingConfig(), plateau_patience=0, plateau_cooldown=0)
        optimizer = _role_optimizer(config)
...
class GTRGuardedEarlyStopTests(unittest.TestCase):
    def test_stale_patience_alone_cannot_stop_before_two_lr_reductions(self):
        config = GTRTrainingConfig(patience=20, min_lr_reductions_before_stop=2)
        self.assertFalse(should_stop_gtr(stale=20, lr_reductions=0, config=config))
        self.assertFalse(should_stop_gtr(stale=40, lr_reductions=1, config=config))
        self.assertTrue(should_stop_gtr(stale=20, lr_reductions=2, config=config))

    def test_fewer_than_twenty_stale_epochs_never_stops(self):
        config = GTRTrainingConfig(patience=20, min_lr_reductions_before_stop=2)
        self.assertFalse(should_stop_gtr(stale=19, lr_reductions=9, config=config))
