...
from bstalignment.v2.battery_cache import (
    BatteryCellFeatures,
    BatteryFeatureCache,
    BatteryOperatingContext,
)
from bstalignment.v2.battery_features import (
    BASE_FEATURE_NAMES,
    CURVE_POINTS,
    DV_NORMALIZED_Q_AXIS,
    IC_VOLTAGE_AXIS,
)
from bstalignment.v2.battery_data import BatteryForecastV2Dataset, BatterySplitV2
import bstalignment.v2.precompute_battery as battery_precompute
from bstalignment.v2.precompute_battery import (
    mit_operating_context,
    source_operating_contexts,
    xjtu_operating_context,
)
from bstalignment.v2.prompts import build_battery_prompt, fit_battery_prompt_thresholds
...
class XJTUProtocolSplitTests(unittest.TestCase):
    ...
    def test_split_is_deterministic_disjoint_and_covers_all_six_protocols(self):
        with TemporaryDirectory() as temporary:
            ...
            split = BatterySplitV2.from_cache(cache, seed=42)
            repeated = BatterySplitV2.from_cache(cache, seed=42)
            other_seed = BatterySplitV2.from_cache(cache, seed=43)
    ...
    def test_invalid_xjtu_context_or_group_shape_fails_loud(self):
        with TemporaryDirectory() as temporary:
            ...
            with self.assertRaisesRegex(ValueError, "requires operating context"):
                BatterySplitV2.from_cache(missing_context, seed=42)
            with self.assertRaisesRegex(ValueError, "six canonical protocol groups"):
                BatterySplitV2.from_cache(bad_shape, seed=42)
    ...
    def test_non_xjtu_cache_keeps_legacy_cell_random_split(self):
        with TemporaryDirectory() as temporary:
            ...
            self.assertEqual(
                BatterySplitV2.from_cache(cache, seed=42),
                BatterySplitV2.from_cell_ids(cache.cell_ids, seed=42),
            )
...
class BatteryPromptThresholdTests(unittest.TestCase):
    ...
    def test_dataset_thresholds_ignore_validation_and_test_cells(self):
        ...
        split = BatterySplitV2.from_cell_ids(ids, seed=42)
        with TemporaryDirectory() as temporary:
            root = Path(temporary)
            ...
            dataset_a = BatteryForecastV2Dataset(cache_a, split="train", seed=42)
            dataset_b = BatteryForecastV2Dataset(cache_b, split="train", seed=42)
            ...
            repeated = BatteryForecastV2Dataset(cache_a, split="train", seed=42)
    ...
    def test_recent_graph_cycles_and_future_targets_do_not_change_prompt(self):
        ...
        with TemporaryDirectory() as temporary:
            ...
            dataset_a = BatteryForecastV2Dataset(
                cache_a,
                split="train",
                seed=42,
            ...
            dataset_b = BatteryForecastV2Dataset(
                cache_b,
                split="train",
                seed=42,
                prompt_thresholds=thresholds,
